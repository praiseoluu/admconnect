...
export default class HomePage extends WebLayout {
      ...
      ${this._reels.length ? `
        <section class="home-section" aria-labelledby="reels-heading">
          <div class="home-section__header">
            ...
            <a href="/south/reels" class="home-section__link">${this.esc(t('home.seeAll'))}</a>
          </div>
      ...
      <div class="home-bottom-row">
        ...
        <section class="home-section home-section--news" aria-labelledby="news-heading">
          <div class="home-section__header">
            ...
            <a href="/south/news" class="home-section__link">${this.esc(t('home.seeAll'))}</a>
          </div>
  ...
  _bindCarousel(total) {
    ...
    this.delegate('.home-carousel__slide', 'click', (e, slide) => {
      ...
      if (slug) router.push(`/south/news/${slug}`);
    });
  ...
  async _mountReelCards() {
    ...
    for (const reel of this._reels) {
      ...
      const card = this.addChild(new ReelCard({
        ...reel,
        onClick: () => router.push(`/south/reels/${reel.reelId}`),
      }));
  ...
  async _mountNewsCards(items) {
    ...
    for (const item of items) {
      ...
      const card = this.addChild(new NewsCard({
        ...item,
        layout: 'horizontal',
        onClick: () => router.push(`/south/news/${item.slug || item.id}`),
      }));
      await card.mount(wrap);
    }
  }
  ...
  _bindChatBtn() {
    ...
    if (btn) this.on(btn, 'click', () => router.push('/south/chat'));
  }
